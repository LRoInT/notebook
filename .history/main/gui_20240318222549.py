...
class NoteBookGUI:  # 窗口类
    ...
    def windows_init(self):
        ...
        self.optionFrame = Frame(self.root, width=100,
                                 height=500, relief='groove', bd=2)
        self.mainFrame = Frame(self.root, width=800,
                               height=500, relief='groove', bd=2)  # 显示界面
        self.optionFrame.place(relx=0.095, rely=0.05, anchor="nw")
        for b in range(len(self.widget_dict["page_option"])):  # 填充控件
            self.widget_dict["page_option"].place(relx=0.1, rely=0.05+0.75*b)
        self.mainFrame.place(relx=0.35, rely=0.05, anchor="nw")
        self.root.mainloop()
