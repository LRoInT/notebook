...
class NoteBook:
    ...
    def _add_plugin(self, plugin_code):
        scope={} #设置作用域
        exec(plugin_code,scope)
        plugin=[]
        plugin_name=scope["name"]
        plugin.append(scope[plugin_name+"Run"]())
        if "__other__" in scope:
            for a in scope["__other__"]:
                plugin[plugin_name].append(scope[a]
    def plugin_set(self, plugin):
        if os.path.exists(plugin):#确认文件存在
            if os.path.isfile(plugin): #当插件为单个文件时
                ...
                plugin_name=scope["name"]
                plugin_func=scope[plugin_name+"Run"]()
                ...
                if os.path.exists(plugin+"\\plugin.json"):
                    plugin_file=json.load(open(plugin+"\\plugin.json",encoding="utf-8"))
